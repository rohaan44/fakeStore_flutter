...
class DummyButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: isLoading || onPressed == null ? null : onPressed, // Disable tap when loading or onPressed is null
      child: Container(
        decoration: BoxDecoration(
          color: (isLoading || onPressed == null)
              ? Colors.grey // Disable color if loading or onPressed is null
              : Color(0xFF704F38), // Default enabled color
        ...
        child: Center(
          child: isLoading
              ? CircularProgressIndicator(
                  valueColor: AlwaysStoppedAnimation<Color>(ColorsCustom.white), // White loading indicator
                )
              : Text(
                  label,
                  style: TextStyle(
